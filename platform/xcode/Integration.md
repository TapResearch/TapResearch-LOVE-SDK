...
To integrate the TapResearchSDK we need to include the SDK, add the TapResearch LÖVE bridge source files, register the native Lua module, and modify the game’s Lua code. The recommended path is to run the installer script so the engine source changes are applied automatically.

## Steps to add TapResearch

Download and unarchive the `TapResearch-LOVE-SDK.zip` file and open the folder.

### Install the LÖVE bridge

From the root of the unarchived `TapResearch-LOVE-SDK` folder, run:
...
- copy the native bridge source folder to `/path/to/love/src/modules/tapresearch`
- patch `/path/to/love/src/modules/love/love.cpp` so `require("tapresearch_native")` works in static LÖVE builds
- copy `TapResearchSDK.xcframework` into `/path/to/love/platform/xcode/ios/libraries` when the framework is included in this SDK package
- update `platform/xcode/liblove.xcodeproj` so the TapResearch bridge files are included in the `liblove-ios` target
- update the Xcode projects so `TapResearchSDK.xcframework` is linked and embedded for iOS builds
- copy `tapresearch.lua` into your game folder when `--game-root` is provided

You can safely run the installer more than once. It will not duplicate the `love.cpp` registration.

Use `--dry-run` to preview changes:

```sh
ruby tools/install_tapresearch_love.rb --love-root /path/to/love --game-root /path/to/your/game --dry-run
```

If you only want to patch the engine source or only want to copy files, run the installer with `--help` to see the available skip options.

After running the installer, open the Xcode project and build the iOS target.

### Manual fallback

If you cannot run the installer, apply the following steps manually.
